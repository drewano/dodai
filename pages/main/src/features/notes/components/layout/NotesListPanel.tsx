...
export interface NotesListPanelProps {
  ...
  onAddNote: (noteData: Omit<NoteEntry, 'createdAt' | 'updatedAt' | 'type'>) => Promise<string>;
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      if (overId.startsWith('droppable-')) {
        ...
        // Convert note to folder while preserving its content
        let preservedContentNoteId: string | null = null;

        // If the target note has non-empty content, create a new note to preserve it
        if (targetNote.content && targetNote.content.trim() !== '') {
          const preservedContentTitle = `Contenu de ${targetNote.title || 'Note sans titre'}`;
          preservedContentNoteId = await onAddNote({
            id: '', // Will be generated by addNote
            title: preservedContentTitle,
            content: targetNote.content,
            sourceUrl: targetNote.sourceUrl,
            tags: targetNote.tags ? [...targetNote.tags] : undefined,
            parentId: targetNote.parentId, // Initially same parent as target note
          });
        }

        // Convert target note to folder
        await onUpdateNote(targetNote.id, {
          type: 'folder',
          content: '', // Clear content as it becomes a folder
        });
        targetFolderIdForDrop = targetNote.id; // Target is the new folder

        // If we created a preserved content note, move it into the new folder
        if (preservedContentNoteId) {
          await onMoveNoteToFolder(preservedContentNoteId, targetFolderIdForDrop);
        }
      }
    ...
    [notes, onMoveNoteToFolder, onMoveFolder, onUpdateNote, onAddNote],
  );
